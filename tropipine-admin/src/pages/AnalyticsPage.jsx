...
import { card, brandGrad } from '../utils/ui';
import {
          ...
          <div className="flex gap-2">
            {PERIODS.map((p) => (
              <button
                ...
                style={period === p.value ? { ...brandGrad, color: '#fff', borderColor: 'transparent' } : { background: '#fff', color: '#6B7280', borderColor: '#E8E8F0' }}
              >
                {p.label}
              </button>
            ))}
                ...
                {topProducts.length > 0 ? (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={topProducts} layout="vertical" barSize={20}>
                      ...
                      <CartesianGrid strokeDasharray="3 3" stroke="#F0F0F5" vertical={true} horizontal={false} />
                      <XAxis type="number" tick={{ fontSize: 11, fill: '#9CA3AF' }} axisLine={false} tickLine={false} />
                      <YAxis dataKey="name" type="category" tick={{ fontSize: 11, fill: '#9CA3AF' }} width={90} axisLine={false} tickLine={false} />
                      <Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid #E8E8F0' }} formatter={(v) => [v, 'Units']} />
                ...
                {lowStockProducts.length === 0 ? (
                  ...
                  <div className="space-y-3 max-h-52 overflow-y-auto">
                    {lowStockProducts.map((p) => (
                      <div key={p.id} className="flex items-center justify-between px-4 py-3 rounded-xl" style={{ background: '#FFFBEB', border: '1px solid #FDE68A' }}>
                        <div>
