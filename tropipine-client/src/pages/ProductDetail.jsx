...
export default function ProductDetail() {
  ...
  const [reviewPreviews, setReviewPreviews] = useState([]) // object URL[]
  ...
  useEffect(() => () => reviewPreviews.forEach(URL.revokeObjectURL), [reviewPreviews])
  ...
  const handleWishlist = () => {
    ...
    else dispatch(addToWishlist({ productId: product.id, name: product.name, price: product.finalPrice, image: selectedImage }))
  }

  const handleFileChange = (e) => {
    const files = Array.from(e.target.files || [])
    const combined = [...reviewFiles, ...files].slice(0, 5)
    setReviewFiles(combined)
    setReviewPreviews(combined.map((f) => URL.createObjectURL(f)))
    e.target.value = ''
  }

  const removeFile = (idx) => {
    URL.revokeObjectURL(reviewPreviews[idx])
    const files = reviewFiles.filter((_, i) => i !== idx)
    const previews = reviewPreviews.filter((_, i) => i !== idx)
  ...
  const handleSubmitReview = async (e) => {
    ...
    if (!user && !guestName.trim()) { setReviewError('Please enter your name'); return }
  ...
  const inputCls = 'w-full px-4 py-2.5 bg-cream border border-stone text-bark text-sm placeholder-sand focus:outline-none focus:border-bark transition-colors'
  ...
  return (
    <div className="min-h-screen bg-cream pt-20">
      ...
      {lightbox && (
        ...
        >
          <img src={lightbox} alt="" className="max-h-[90vh] max-w-full object-contain shadow-2xl" />
          <button className="absolute top-4 right-4 text-white/70 hover:text-white text-3xl leading-none" onClick={() => setLightbox(null)}>×</button>
        </div>
      )}

      <div className="max-w-7xl mx-auto px-6 sm:px-10 py-14 space-y-16">
        ...
        <div className="grid lg:grid-cols-2 gap-14 lg:gap-20">
          ...
          <div className="flex flex-col">
            {product.category && <p className="label text-grove mb-3">{product.category.name}</p>}
            ...
            {product.origin && (
              <p className="text-xs text-clay/70 mb-5"><span className="font-medium text-bark">Origin:</span> {product.origin}</p>
            )}
            ...
            <div className="space-y-2 mb-6">
              ...
              <button
                ...
                className={`w-full py-3.5 border text-sm font-medium tracking-wide transition-colors ${
                  isInWishlist ? 'bg-bark text-white border-bark' : 'border-stone text-clay hover:border-bark hover:text-bark'
                }`}
              >
        ...
        <div className="border-t border-stone pt-14">
          ...
          <div className="grid lg:grid-cols-2 gap-14">
            ...
            <div className="space-y-5">
              {reviewsLoading ? (
                [...Array(2)].map((_, i) => <div key={i} className="h-24 bg-white border border-stone animate-pulse" />)
              ) : reviews.length === 0 ? (
                ...
                const name = review.user?.name || review.guestName || 'Anonymous'
                return (
                  <div key={review.id} className="bg-white border border-stone p-6">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-bark flex items-center justify-center text-white text-xs font-medium flex-shrink-0">
                          {name[0]?.toUpperCase() || '?'}
                        </div>
                        <div>
                          <p className="text-sm font-medium text-bark">{name}</p>
                    ...
                    {review.comment && (
                      <p className="text-sm text-clay leading-relaxed italic mb-3">"{review.comment}"</p>
                    )}
                      ...
                      <div className="flex gap-2 flex-wrap mt-2">
                        {review.images.map((url, i) => (
                          <button
                            ...
                            className="w-16 h-16 overflow-hidden border border-stone hover:border-bark transition-colors flex-shrink-0"
                          >
            ...
            <div className="bg-white border border-stone p-8">
              <h3 className="font-display text-xl font-semibold text-bark mb-1">Write a Review</h3>
              <p className="text-xs text-clay/60 mb-6">Anyone can leave a review — no account needed.</p>

              {hasReviewed || reviewSuccess ? (
                <div className="text-center py-8">
                  <div className="w-12 h-12 bg-grove/10 rounded-full flex items-center justify-center mx-auto mb-3">
                    <svg className="w-6 h-6 text-grove" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <form onSubmit={handleSubmitReview} className="space-y-4">
                  ...
                  {/* Guest fields */}
                  {!user && (
                    <div className="space-y-3 pb-4 border-b border-stone">
                      <div>
                        <label className="label text-clay/70 block mb-1.5">Your Name <span className="text-earth">*</span></label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="label text-clay/70 block mb-1.5">Email <span className="text-clay/40">(optional)</span></label>
                        <input
                          type="email"
                  ...
                  {/* Rating */}
                  <div>
                    <label className="label text-clay/70 block mb-2">Rating <span className="text-earth">*</span></label>
                    <StarPicker value={reviewRating} onChange={setReviewRating} />
                  </div>

                  {/* Comment */}
                  <div>
                    <label className="label text-clay/70 block mb-1.5">Comment <span className="text-clay/40">(optional)</span></label>
                    <textarea
                      value={reviewComment}
                  ...
                  {/* Image upload */}
                  <div>
                    <label className="label text-clay/70 block mb-2">Photos <span className="text-clay/40">(up to 5)</span></label>
                      ...
                      <div className="flex gap-2 flex-wrap mb-2">
                        {reviewPreviews.map((src, i) => (
                          <div key={i} className="relative group w-16 h-16 flex-shrink-0">
                            <img src={src} alt="" className="w-full h-full object-cover border border-stone" />
                            <button
                              type="button"
                              onClick={() => removeFile(i)}
                              className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-bark text-white rounded-full text-[10px] flex items-center justify-center leading-none opacity-0 group-hover:opacity-100 transition-opacity"
                            >×</button>
                          </div>
                        ))}
                      </div>
                    ...
                    {reviewFiles.length < 5 && (
                      <>
                        ...
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 border border-dashed border-stone text-clay text-xs hover:border-bark hover:text-bark transition-colors"
                        >
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                          </svg>
                          Add photos{reviewFiles.length > 0 ? ` (${reviewFiles.length}/5)` : ''}
                        </button>
                      </>
                    )}
                  </div>
                  ...
                  {reviewError && (
                    <p className="text-earth text-xs">{reviewError}</p>
                  )}
